...
class App extends Component {
  ...
  calculateFaceLocation = (data) => {
    ...
    const clarifaiFace = data.outputs[0].data.regions.map(region=>
      {
        const value = region.region_info.bounding_box;
        return {
  ...
  onButtonSubmit = () => {
    ...
    fetch('https://intense-depths-99650.herokuapp.com/imageurls', {
      method: 'post',
    ...
    .then(response => response.json())
      .then(response => {
        if (response) {
          fetch('https://intense-depths-99650.herokuapp.com/image', {
            method: 'put',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
